 ...
 */
public final class JavaSVMLoad {
  ...
  public static void main(String[] args) {
	    ...
	    SVMModel model=SVMModel.load(sc.sc(), "/ntes_weblog/sparkModel/SVMModel");
	    JavaPairRDD<Text,Text>  webInfoResultPercent = sc.sequenceFile("/ntes_weblog/portalUserAnalysis/statistics/userProfile/weeklyWebInfoResult/20160324/", Text.class, Text.class);
	    JavaRDD<LabeledPoint> jpoints =  webInfoResultPercent.map(new Function<Tuple2<Text,Text>,LabeledPoint>(){
	     ...
	     // JavaRDD<Double>  prediction = model.predict(testv) ; 
	      final Broadcast<SVMModel> bmodel = sc.broadcast(model);
	      JavaRDD<Integer> labels = test.map(new Function<LabeledPoint, Integer>(){
	    	  SVMModel lmodel = bmodel.getValue();
